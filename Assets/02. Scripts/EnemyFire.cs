...
public class EnemyFire : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        muzzleFlash.enabled = false; //Mesh Renderer가 꺼짐
    }

    // Update is called once per frame
    void Update()
    {
        if(!isReload && isFire) //빈도수 적은 이벤트를 앞으로
        {
    ...
    IEnumerator ShowMuzzleFlash()
    {
        ...
        //머즐플래쉬 쿼드를 0~360도 회전 시키기 위함
        Quaternion rot = Quaternion.Euler(Vector3.forward * Random.Range(0, 360));
        muzzleFlash.transform.localRotation = rot;

        //머즐플래시의 Scale값을 xyz 모두 1~2배 늘리기
        muzzleFlash.transform.localScale = Vector3.one * Random.Range(1f, 2f);

        //텍스처 오프셋 조정하기
        //Random.Range에 의해서 0 또는 1 값이 나오는데 0.5를 곱해서 0 또는 0.5 값이 나오도록 계산
